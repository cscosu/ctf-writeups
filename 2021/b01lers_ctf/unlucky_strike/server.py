...
def unpkcs7(padded):
    while True:
        if (len(padded) & 0xF) != 0 or len(padded) == 0:
            break
        padlen = padded[-1]
        if padlen < 1 or padlen > 16:
            break
        if any([padded[-(i + 1)] != padlen for i in range(padlen)]):
            break
...
class Server:
    ...
    def main(self):
        ...
        sys.stdout.flush()
        self.numbers = [
            int.from_bytes(get_random_bytes(3), "big") for i in range(Nballs)
        ]
